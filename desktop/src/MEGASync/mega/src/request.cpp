...
bool Request::processSeqTag(Command* cmd,
                            bool withJSON,
                            bool& parsedOk,
                            bool inSeqTagArray,
                            JSON& processingJson)
{
    ...
    if (inSeqTagArray)
    {
        if (*processingJson.pos == ',')
            ++processingJson.pos;
    }

    // In normal operation, we get called once to figure out the `st` to watch out for in sc json
    // (else case below) then the sc processing will process actionpackets up to and including the
    // one(s) with that `st` and only after that, call us a second time (with mCurrentSeqtagSeen
    // true), and we will execute the if block. And one additional case to consider, before we start
    // the sc channel, it's ok to send non-actionpacket (ie, non-st) requests (hence scsn check)

    if (cmd->client->mCurrentSeqtag == st ||
        !cmd->client->scsn.ready()) // if we have not started the sc channel, we won't receive the
                                    // matching st, so ignore st until we do
    {
        if (!cmd->client->scsn.ready())
        {
            LOG_verbose << "Processing command result regardless of st as we don't have scsn yet. "
                        << st << cmd->client->mCurrentSeqtag;
        }
        ...
        parsedOk =
            withJSON ?
                processCmdJSON(cmd, false, processingJson) :
                cmd->procresult(Command::Result(Command::CmdError, API_OK),
                                processingJson); // just an `st` returned is implicitly successful
        return true;
    }
    else
    {
        // result processing paused until we encounter and process actionpackets matching
        // client->mCurrentSeqtag
...
void Request::process(MegaClient* client)
{
    ...
    for (; processindex < cmds.size() && !stopProcessing; processindex++)
    {
        ...
        if (cmd->mSeqtagArray && processingJson.enterarray())
        {
            ...
            // So in the case of success with a string return, but no `st`, the array is [0,
            // "returnValue"] If the command failed, there is no array, just the error code
            assert(cmd->mV3);
            ...
            else if (!processSeqTag(cmd,
                                    true,
                                    parsedOk,
                                    true,
                                    processingJson)) // executes the command's procresult if we
                                                     // match the seqtag
            {
        ...
        if (!parsedOk)
        {
            ...
            // alert devs to the JSON problem (bad JSON from server, or bad parsing of it)
            // immediately
            assert(false);
        }
        else
...
void Request::swap(Request& r)
{
    ...
    // If swap was used when these were active, we would be moving needed info out of the
    // request-in-progress
...
bool RequestDispatcher::readyToSend() const
{
    ...
    {
        return nextreqs.empty() ? false : !nextreqs.front().empty();
    }
}
...
void RequestDispatcher::clear()
{
    if (processing)
    {
        // we are being called from a command that is in progress (eg. logout) - delay wiping the
        // data structure until that call ends.
        clearWhenSafe = true;
