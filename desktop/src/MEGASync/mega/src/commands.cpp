        ...
        return new CommandPutFA(
            ...
            [weakSelf, client](Error e, const std::string& url, const vector<std::string>& /*ips*/)
            {
                ...
                if (e == API_OK)
                {
                    ...
                    self->post(client,
                               self->data->data(),
                               static_cast<unsigned>(self->data->size()));
                }
                else
                {
                    // jumping to REQ_SUCCESS, but with no handle in `in`, means we failed overall
                    // (and don't retry)
...
bool CommandPutFA::procresult(Result r, JSON& json)
{
    if (r.wasErrorOrOK())
    {
        ...
        if (r.wasError(API_EACCESS))
        {
            ...
            {
                LOG_debug << "Restoration of file attributes is not allowed for current user ("
                          << me64 << ").";

                // 'canChangeVault' is false here because restoration of file attributes is
                // triggered by downloads, so it cannot be triggered by a Backup operation
            ...
            switch (json.getnameid())
            {
                ...
                case EOO:
                    ...
                    else
                    {
                        ...
                        {
                            LOG_err << "Unpaired IPs received for URLs in `ufa` command. URLs: "
                                    << urls.size() << " IPs: " << ips.size();
                        }
        ...
        switch (json.getnameid())
        {
            ...
            case EOO:
                if (it != client->fafcs.end())
                {
                    ...
                    {
                        faf_map::iterator fafsit;
                        for (fafsit = it->second->fafs[0].begin();
                             fafsit != it->second->fafs[0].end();)
                        {
                            // move from fresh to pending
                            it->second->fafs[1][fafsit->first] = fafsit->second;
                            it->second->fafs[0].erase(fafsit++);
...
CommandPutFile::CommandPutFile(MegaClient* client, TransferSlot* ctslot)
{
    ...
    {
        // Target user goes alone, not inside an array. Note: we are skipping this if a)more than
        // two b)the array had been created for node handles
        for (auto& file: tslot->transfer->files)
        {
        ...
        switch (json.getnameid())
        {
            ...
            case EOO:
                ...
                if (tempurls.size() == 1)
                {
                    ...
                    {
                        LOG_err << "Unpaired IPs received for URLs in `u` command. URLs: "
...
CommandDirectRead::CommandDirectRead(MegaClient* client, DirectReadNode* cdrn)
{
    ...
    arg("g", 1); // server will provide download URL(s)/token(s) (if skipped, only information about
                 // the file)
...
bool CommandDirectRead::procresult(Result r, JSON& json)
{
    ...
    else
    {
        ...
        for (;;)
        {
            ...
            {
                case makeNameid("g"):
                    if (json.enterarray()) // now that we are requesting v2, the reply will be an
                                           // array of 6 URLs for a raid download, or a single URL
                                           // for the original direct download
                    {
                        for (;;)
                        {
                            std::string tu;
...
bool CommandGetFile::procresult(Result r, JSON& json)
{
    ...
    for (;;)
    {
        ...
        {
            case makeNameid("g"):
                if (json.enterarray()) // now that we are requesting v2, the reply will be an array
                                       // of 6 URLs for a raid download, or a single URL for the
            ...
            case EOO:
            {
                ...
                SymmCipher* cipherer =
                    client->getRecycledTemporaryTransferCipher(filekey, mFileKeyType);
...
{
    // We generate the command just before sending, so it's up to date for any external changes that
    // occured in the meantime And we can also take into account any changes we have sent for this
    // node that have not yet been applied by actionpackets
    jsonWriter.clear();
    generationError = API_OK;
    ...
    if (shared_ptr<Node> n = clientOfRequest->nodeByHandle(h))
    {
        ...
        n->mPendingChanges.forEachCommand(
            [&m, this](Command* cmd)
            {
                if (cmd == this)
                    return;
...
bool CommandSetAttr::procresult(Result r, JSON&)
{
    ...
    if (completion)
        completion(h, generationError ? Error(generationError) : r.errorOrOK());
    ...
    mResultFunction(resultFunction)
{
    ...
    for (auto& n: newnodes)
        assert(n.canChangeVault == canChangeVault);
#endif
    ...
    // "vb": when provided, it force to override the account-wide versioning behavior by the value
    // indicated by client
    //     vb:1 to force it on
    //     vb:0 to force it off
    // Dont provide it at all to rely on the account-wide setting (as of the moment the command is
    // processed).
    ...
    for (unsigned i = 0; i < nn.size(); i++)
    {
        ...
        switch (nni->source)
        {
            ...
            case NEW_UPLOAD:
                ...
                {
                    // if attributes are set on the newnode then the app is not using the
                    // pendingattr mechanism
        ...
        if (vo != NoVersioning && nn[i].type == FILENODE && !nn[i].ovhandle.isUndef())
        {
        ...
        {
            assert(!nn[i].hasZeroKey()); // Add this assert here to avoid extra checks in production
                                         // -> we will have a check and logs if this fails within
                                         // CommandPutNode::procresult
        ...
        if ((tn = client->nodeByHandle(th)))
        {
            ...
            for (unsigned i = 0; i < nn.size(); i++)
            {
                switch (nn[i].source)
                {
                    ...
                    case NEW_UPLOAD:
                        snk.add(nn[i].nodekey,
                                nn[i].nodehandle,
                                tn,
                                true,
                                nn[i].uploadtoken.data(),
                                (int)sizeof nn[i].uploadtoken);
                        break;
                }
            }
...
CommandMoveNode::CommandMoveNode(MegaClient* client,
                                 std::shared_ptr<Node> n,
                                 ...
                                 bool canChangeVault)
{
    ...
    // This is needed for backward compatibility, old versions used memcmp to detect if a 'd'
    // actionpacket was followed by a 't'  actionpacket with the same 'i' (ie, a move) Additionally
    // the servers can't deliver `st` in that packet for the same reason.  And of course we will not
    // ignore this `t` packet, despite setting 'i'.
    notself(client);
...
bool CommandDelNode::procresult(Result r, JSON& json)
{
    ...
    {
        if (mResultFunction)
            mResultFunction(h, r.errorOrOK());
        else
            client->app->unlink_result(h.as8byte(), r.errorOrOK());
        return true;
    }
    else
    {
            ...
            switch (json.getnameid())
            {
                ...
                case EOO:
                    if (mResultFunction)
                        ...
                        client->app->unlink_result(h.as8byte(), e);
                    return true;
                ...
                default:
                    ...
                    {
                        if (mResultFunction)
                            mResultFunction(h, API_EINTERNAL);
                        else
                            client->app->unlink_result(h.as8byte(), API_EINTERNAL);
                        return false;
                    }
            }
        }
...
const char* CommandLogout::getJSON(MegaClient* clientOfRequest)
{
    if (!incrementedCount)
    {
        // only set this once we are about to send the command, in case there are others ahead of it
        // in the queue
...
bool CommandLogout::procresult(Result r, JSON&)
{
    ...
    if (r.wasError(API_OK))
    {
        ...
        LOG_debug << "setting mOnCSCompletion for final logout processing"; // track possible lack
                                                                            // of logout callbacks
    ...
    email(email)
{
    ...
    arg("user", email);
    batchSeparately =
        true; // in case the account is blocked (we need to get a sid so we can issue whyamiblocked)
    ...
    mCompletion(std::move(completion))
{
    ...
    LOG_debug << "CommandLogin:" << getJSON(client);
    tag = client->reqtag;
}
        ...
        switch (json.getnameid())
        {
            ...
            case EOO:
                ...
                { // scope for local variable
                    MegaClient* cl = client; // make a copy, because 'this' will be gone by the time
                                             // lambda will execute
                    client->loginResult(std::move(mCompletion),
                                        API_OK,
                                        [cl]()
                                        {
                                            cl->getaccountdetails(
...
CommandSetShare::CommandSetShare(MegaClient* client,
                                 std::shared_ptr<Node> n,
                                 User* u,
                                 accesslevel_t a,
                                 bool newshare,
                                 ...
                                 int ctag,
                                 std::function<void(Error, bool writable)> f)
{
    ...
    arg("u",
        u ? ((u->show == VISIBLE) ? u->uid.c_str() : u->email.c_str()) : MegaClient::EXPORTEDLINK);
    // if the email is registered, the pubk request has returned the userhandle -->
    // sending the userhandle instead of the email makes the API to assume the user is already a
    // contact
...
CommandPendingKeys::CommandPendingKeys(MegaClient* client,
                                       CommandPendingKeysReadCompletion completion)
{
...
bool CommandPendingKeys::procresult(Result r, JSON& json)
{
    ...
    std::shared_ptr<map<handle, map<handle, string>>> keys =
        std::make_shared<map<handle, map<handle, string>>>();
    std::string lastcompleted;
...
CommandSetPendingContact::CommandSetPendingContact(MegaClient* client,
                                                   const char* temail,
                                                   opcactions_t action,
                                                   const char* msg,
                                                   const char* oemail,
    ...
    if (r.wasErrorOrOK())
    {
        ...
        if (r.wasError(API_OK)) // response for delete & remind actions is always numeric
        {
            ...
            else if (action == OPCA_DELETE)
            {
                ...
                {
                    if (n->pendingshares &&
                        n->pendingshares->find(pcr->id) != n->pendingshares->end())
                    {
                        client->newshares.push_back(new NewShare(n->nodehandle,
                                                                 1,
                                                                 n->owner,
                                                                 ACCESS_UNKNOWN,
                                                                 0,
...
CommandUpdatePendingContact::CommandUpdatePendingContact(MegaClient* client,
                                                         handle p,
                                                         ipcactions_t action,
                                                         Completion completion)
{
...
bool CommandEnumerateQuotaItems::procresult(Result r, JSON& json)
{
    ...
    while (json.enterobject())
    {
        ...
        unsigned int testCategory =
            CommandEnumerateQuotaItems::INVALID_TEST_CATEGORY; // Bitmap. Bit 0 set (int value 1) is
                                                               // standard plan, other bits are
                                                               // defined by API.
        ...
        while (!finished)
        {
            ...
            switch (json.getnameid())
            {
                    ...
                    while (!finished)
                    {
                        ...
                        switch (json.getnameid())
                        {
                            ...
                            case EOO:
                                ...
                                if (currencyData->currencyName.empty() ||
                                    currencyData->currencySymbol.empty())
                                {
                ...
                case makeNameid("it"): // 0 -> for all Pro level plans; 1 -> for Business plan;
                    ...
                    break;
                    // case makeNameid("ib"): // for "it":1 (business plans), 0 -> Pro Flexi; 1 ->
                    // Business plan
                    //     {
                ...
                case makeNameid("f"): // e.g. "f": { "vpn": 1 }
                {
                    ...
                    {
                        LOG_err
                            << "Failed to parse Enumerate-quota-items response, enter `f` object";
                        client->app->enumeratequotaitems_result(API_EINTERNAL);
                        return false;
                    }
                    string key, value;
                    ...
                    {
                        LOG_err
                            << "Failed to parse Enumerate-quota-items response, leave `f` object";
                        client->app->enumeratequotaitems_result(API_EINTERNAL);
                        return false;
                    }
                    break;
                        ...
                        switch (json.getnameid())
                        {
                            case makeNameid("ba"): // base (-1 means unlimited storage or transfer)
                            {
                                ...
                                while (readingBa)
                                {
                                    ...
                                    {
                                        case makeNameid("s"):
                                            bizPlan->gbStoragePerUser =
                                                static_cast<int>(json.getint());
                                            break;
                                        case makeNameid("t"):
                                            bizPlan->gbTransferPerUser =
                                                static_cast<int>(json.getint());
                                            break;
                                        case EOO:
                                            readingBa = false;
                                        ...
                                        default:
                                            if (!json.storeobject())
                                            {
                                                LOG_err << "Failed to parse Enumerate-quota-items "
                                                           "response, `ba` data";
                            ...
                            case makeNameid("us"): // price per user
                            {
                                ...
                                while (readingUs)
                                {
                                    ...
                                    {
                                        case makeNameid("p"):
                                            bizPlan->pricePerUser =
                                                static_cast<unsigned>(json.getint());
                                            break;
                                        case makeNameid("lp"):
                                            bizPlan->localPricePerUser =
                                                static_cast<unsigned>(json.getint());
                                            break;
                                        case EOO:
                                            readingUs = false;
                                ...
                                while (readingSto)
                                {
                                    switch (json.getnameid())
                                    {
                                        case makeNameid("s"):
                                            ...
                                            break;
                                        case makeNameid("p"):
                                            bizPlan->pricePerStorage =
                                                static_cast<unsigned>(json.getint());
                                            break;
                                        case makeNameid("lp"):
                                            bizPlan->localPricePerStorage =
                                                static_cast<unsigned>(json.getint());
                                            break;
                                        case EOO:
                                            readingSto = false;
                            ...
                            case makeNameid("trns"): // transfer block
                            {
                                ...
                                while (readingTrns)
                                {
                                    ...
                                    {
                                        case makeNameid("t"):
                                            bizPlan->gbPerTransfer =
                                                static_cast<int>(json.getint());
                                            break;
                                        case makeNameid("p"):
                                            bizPlan->pricePerTransfer =
                                                static_cast<unsigned>(json.getint());
                                            break;
                                        case makeNameid("lp"):
                                            bizPlan->localPricePerTransfer =
                                                static_cast<unsigned>(json.getint());
                                            break;
                                        case EOO:
                                            readingTrns = false;
...
CommandPurchaseAddItem::CommandPurchaseAddItem(MegaClient* client,
                                               int itemclass,
                                               handle item,
                                               unsigned price,
                                               const char* currency,
...
CommandPurchaseCheckout::CommandPurchaseCheckout(MegaClient* client, int gateway)
{
    ...
    beginarray("s");
    for (handle_vector::iterator it = client->purchase_basket.begin();
         it != client->purchase_basket.end();
         it++)
    {
...
CommandRemoveContact::CommandRemoveContact(MegaClient* client,
                                           const char* m,
                                           visibility_t show,
                                           Completion completion)
{
...
CommandPutMultipleUAVer::CommandPutMultipleUAVer(MegaClient* client,
                                                 const userattr_map* attrs,
                                                 int ctag,
                                                 std::function<void(Error)> completion)
{
    mSeqtagArray = true;
    ...
    mCompletion = completion ? std::move(completion) : [this](Error e)
    {
...
bool CommandPutMultipleUAVer::procresult(Result r, JSON& json)
{
    if (r.hasJsonObject())
    {
        ...
        for (;;) // while there are more attrs to read...
        {
            ...
            {
                LOG_err << "Error in CommandPutMultipleUAVer. Undefined attribute or version: "
                        << key;
                for (auto a: this->attrs)
                {
                    LOG_err << " expected one of: " << User::attr2string(a.first);
                }
                break;
            }
            else
                ...
                else if (type == ATTR_KEYS)
                {
                    if (!client->mKeyManager.fromKeysContainer(it->second))
                    {
                        LOG_err << "Error processing new established value for the Key Manager "
                                   "(CommandPutMultipleUAVer)";
                        // We can't use a previous value here because CommandPutMultipleUAVer is
                        // only used to update ^!keys during initialization
                    }
                }
            }
        }
...
CommandPutUAVer::CommandPutUAVer(MegaClient* client,
                                 attr_t at,
                                 const byte* av,
                                 unsigned avl,
                                 int ctag,
...
CommandPutUA::CommandPutUA(MegaClient* /*client*/,
                           attr_t at,
                           const byte* av,
                           unsigned avl,
                           int ctag,
                           ...
                           CompletionTLV completionTLV)
{
    ...
    mCompletionErr = completionErr ? std::move(completionErr) : [this](error e)
    {
...
bool CommandGetUA::procresult(Result r, JSON& json)
{
    ...
    if (r.wasErrorOrOK())
    {
        ...
        if (u && !u->isTemporary && u->userhandle != client->me && r.wasError(API_ENOENT))
        {
            ...
            {
                LOG_warn << "Missing public key " << User::attr2string(at) << " for user "
                         << u->uid;
                ...
                case EOO:
                {
                    ...
                    switch (User::scope(at))
                    {
                        ...
                        case ATTR_SCOPE_PRIVATE_UNENCRYPTED:
                        {
                            ...
                            {
                                LOG_err << "Error processing new established value for the Key "
                                           "Manager upon init";
                            ...
                            {
                                LOG_info << "CallKit is "
                                         << ((!strcmp(value.data(), "1")) ? "disabled" : "enabled");
                            }
                            break;
                        }
...
CommandKeyCR::CommandKeyCR(MegaClient* /*client*/,
                           sharedNode_vector* rshares,
                           sharedNode_vector* rnodes,
                           const char* keys)
{
    cmd("k");
    beginarray("cr");
...
bool CommandPubKeyRequest::procresult(Result r, JSON& json)
{
    ...
    {
        if (!r.wasError(
                API_ENOENT)) // API_ENOENT = unregistered users or accounts without a public key yet
        {
            ...
            switch (json.getnameid())
            {
                ...
                case EOO:
                    ...
                    if (!ISUNDEF(uh))
                    {
                        ...
                        if (u->isTemporary &&
                            u->uid == u->email) // update uid with the received USERHANDLE (will be
                                                // used as target for putnodes)
                        {
        ...
        switch (json.getnameid())
        {
                                    ...
                                    while (!exit)
                                    {
                                        switch (json.getnameid())
                                        {
                                            ...
                                            case EOO:
                                                ...
                                                {
                                                    LOG_warn << "Unpaired/missing business "
                                                                "status-ts in b.sts";
                                                }
                                                exit = true;
                                                break;
            ...
            case EOO:
            {
                ...
                if (len_privk)
                {
                    ...
                    privk.resize(static_cast<size_t>(
                        Base64::btoa(privkbuf, len_privk, (char*)privk.data())));
                ...
                if (u)
                {
                    ...
                    if (country.size())
                    {
                        changes |= u->updateAttributeIfDifferentVersion(ATTR_COUNTRY,
                                                                        country,
                                                                        versionCountry);
                    }
                    else
                    ...
                    {
                        changes |=
                            u->updateAttributeIfDifferentVersion(ATTR_CONTACT_LINK_VERIFICATION,
                    ...
                    else if (unshareableKey.empty()) // it has not been created yet
                    {
                        ...
                        client->putua(ATTR_UNSHAREABLE_KEY,
                                      newunshareablekey,
                    ...
                    if (!enabledTestNotifications.empty() ||
                        !versionEnabledTestNotifications.empty())
                    {
                        changes |=
                    ...
                    {
                        changes |=
                            u->updateAttributeIfDifferentVersion(ATTR_LAST_READ_NOTIFICATION,
                    ...
                    if (keys.size())
                    {
                        ...
                        if (!client->mKeyManager.fromKeysContainer(keys))
                        {
                            ...
                            // if there's a previous version, better keep that value in cache
                            const UserAttribute* attribute =
                                client->ownuser()->getAttribute(ATTR_KEYS);
                            if (attribute && !attribute->isNotExisting() &&
                                !attribute->version().empty())
                            {
                                LOG_warn << "Replacing ^!keys value by previous version "
                        ...
                        changes |=
                            u->updateAttributeIfDifferentVersion(ATTR_KEYS, keys, keysVersion);
                    }
                    ...
                    {
                        // Process the following ones only when there is no ^!keys yet in the
                        // account. If ^!keys exists, they are all already in it.
                ...
                if (b) // business account
                {
                    ...
                    if ((s < BIZ_STATUS_EXPIRED ||
                         s > BIZ_STATUS_GRACE_PERIOD) // status not received or invalid
                        || (m == BIZ_MODE_UNKNOWN &&
                            !client->isProFlexi())) // master flag not received or invalid (or Pro
                                                    // Flexi, not business)
                    {
                    ...
                    else
                    {
                        for (auto it: sts)
                        {
                            ...
                            {
                                LOG_warn << "Unexpected status in b.sts. Status: " << status
                                         << "ts: " << ts;
                            }
                        }
                        ...
                        // if current business status will expire sooner than the scheduled `ug`,
                        // update the backoff to a shorter one in order to refresh the business
                        // status asap
                        m_time_t auxts = 0;
                        ...
                        if (auxts)
                        {
                            ...
                            // diff < 0 grace period has already expired, ug update is requested one
                            // per day
                        ...
                        // TODO: check if type of account has changed and notify with new event (not
                        // yet supported by API)
                    }
                }
                else
                {
...
void CommandGetUserData::parseUserAttribute(JSON& json,
                                            std::string& value,
                                            std::string& version,
                                            bool asciiToBinary)
{
...
CommandABTestActive::CommandABTestActive(MegaClient* client,
                                         const string& flag,
                                         Completion completion):
    mCompletion(completion)
{
...
CommandGetUserQuota::CommandGetUserQuota(
    MegaClient* client,
    ...
    bool pro,
    int source,
    std::function<void(std::shared_ptr<AccountDetails>, Error)> completion):
    details(ad),
    mStorage(storage),
    mTransfer(transfer),
    mPro(pro),
...
bool CommandGetUserQuota::procresult(Result r, JSON& json)
{
    ...
    for (;;)
    {
        ...
        {
            case makeNameid("bt"):
                // "Base time age", this is number of seconds since the start of the current quota
                // buckets age of transfer window start
                td = json.getint();
                if (td != -1)
                {
...
#ifdef _DEBUG
                        // TODO: remove this debugging block once local count is confirmed to work
                        // correctly 100% verify the new local storage counters per root match
                        // server side (could fail if actionpackets are pending)
                        shared_ptr<Node> node = client->nodebyhandle(h);
                        if (node)
                        {
                            NodeCounter counter = node->getCounter();
                            LOG_debug
                                << node->displaypath() << " " << counter.storage << " " << ns->bytes
                                << " " << counter.files << " " << ns->files << " "
            ...
            case makeNameid("tuo"):
                // Transfer usage by the owner on quotad which hasn't yet been committed back to the
                // API DB. Supplements caxfer
                details->transfer_own_used += json.getint();
                break;
            ...
            case makeNameid("tua"):
                // Transfer usage served to other users which hasn't yet been committed back to the
                // API DB. Supplements csxfer
                details->transfer_srv_used += json.getint();
                break;
            ...
            case makeNameid("uslw"):
                // The percentage (in 1000s) indicating the limit at which you are 'nearly' over.
            ...
            case EOO:
                ...
                client->app->account_details(details.get(),
                                             mStorage,
                                             mTransfer,
                                             mPro,
                                             false,
...
CommandGetUserTransactions::CommandGetUserTransactions(MegaClient* client,
                                                       std::shared_ptr<AccountDetails> ad)
{
    cmd("utt");
...
CommandGetUserPurchases::CommandGetUserPurchases(MegaClient* client,
                                                 std::shared_ptr<AccountDetails> ad)
{
    cmd("utp");
...
CommandGetUserSessions::CommandGetUserSessions(MegaClient* client,
                                               std::shared_ptr<AccountDetails> ad)
{
    cmd("usl");
...
{
    // depending on 'w', the response can be [{"ph":"XXXXXXXX","w":"YYYYYYYYYYYYYYYYYYYYYY"}] or
    // simply [XXXXXXXX]
    if (r.hasJsonObject())
    {
        assert(mWritable);
            ...
            case EOO:
                ...
                if (s >= 0)
                {
                    ...
                    if (op == 2) // importing WelcomePDF for new account
                    {
                        ...
                        client->putnodes(client->mNodeManager.getRootNodeFiles(),
                                         NoVersioning,
                                         std::move(newnodes),
                                         nullptr,
                                         0,
...
CommandAccountVersionUpgrade::CommandAccountVersionUpgrade(vector<byte>&& clRandValue,
                                                           vector<byte>&& encMKey,
                                                           string&& hashedAuthKey,
                                                           string&& salt,
                                                           int ctag,
                                                           std::function<void(error e)> completion):
    mEncryptedMasterKey(std::move(encMKey)),
    mSalt(std::move(salt)),
    mCompletion(completion)
{
    cmd("avu");
    ...
    arg("hak",
        reinterpret_cast<const byte*>(hashedAuthKey.c_str()),
        static_cast<int>(hashedAuthKey.size()));
...
bool CommandAccountVersionUpgrade::procresult(Result r, JSON&)
{
    ...
    if (goodJson)
    {
        if (r.errorOrOK() == API_OK)
        {
            ...
            client->k.assign(reinterpret_cast<const char*>(mEncryptedMasterKey.data()),
                             mEncryptedMasterKey.size());
...
CommandResumeEphemeralSession::CommandResumeEphemeralSession(MegaClient*,
                                                             handle cuh,
                                                             const byte* cpw,
                                                             int ctag)
{
...
{
    cmd("whyamiblocked");
    batchSeparately = true; // don't let any other commands that might get batched with it cause the
                            // whole batch to fail
...
CommandSendSignupLink2::CommandSendSignupLink2(MegaClient* client,
                                               const char* email,
                                               const char* name)
{
    cmd("uc2");
...
CommandSetKeyPair::CommandSetKeyPair(MegaClient* client,
                                     const byte* privk,
                                     unsigned privklen,
                                     const byte* pubk,
                                     unsigned pubklen)
                                     ...
                                     const NodeHandle partialFetchRoot)
{
    ...
    mFilters.emplace("<",
                     [this, client](JSON*)
                     {
                         if (!mFirstChunkProcessed)
                         {
                             ...
                             if (client->sctable)
                             {
                                 // reset sc database for brand new node tree (note that we may be
    ...
    // End of node array
    f = mFilters.emplace("{[f",
                         [this, client](JSON* json)
                         {
                             client->mergenewshares(0);
                             client->mNodeManager.checkOrphanNodes(mMissingParentNodes);

                             // No need to call Syncs::triggerSync here like in
                             // MegaClient::readnodes because it does nothing when
                             // MegaClient::fetchingnodes is true
...
CommandSubmitPurchaseReceipt::CommandSubmitPurchaseReceipt(MegaClient* client,
                                                           int type,
                                                           const char* receipt,
                                                           handle lph,
                                                           int phtype,
...
// Credit Card Store
CommandCreditCardStore::CommandCreditCardStore(MegaClient* client,
                                               const char* cc,
                                               const char* last4,
                                               const char* expm,
                                               const char* expy,
...
CommandCopySession::CommandCopySession(MegaClient* client)
{
    ...
    arg("c", 1);
    batchSeparately = true; // don't let any other commands that might get batched with it cause the
                            // whole batch to fail when blocked
    tag = client->reqtag;
}

// for ephemeral accounts, it returns "tsid" instead of "csid" -> not supported, will return
// API_EINTERNAL
...
CommandSendReport::CommandSendReport(MegaClient* client,
                                     const char* type,
                                     const char* blob,
                                     const char* uid)
{
...
CommandSendEvent::CommandSendEvent(MegaClient* client,
                                   int type,
                                   const char* desc,
                                   bool addJourneyId,
                                   const char* viewId)
{
    ...
    if (addJourneyId)
    {
        ...
        if (!journeyId.empty())
        {
            ...
            m_off_t currentms = std::chrono::duration_cast<std::chrono::milliseconds>(
        ...
        {
            LOG_warn << "[CommandSendEvent::CommandSendEvent] Add JourneyID flag is ON, but there "
                        "is no JourneyID value set";
        }
    }
...
CommandGetRecoveryLink::CommandGetRecoveryLink(MegaClient* client,
                                               const char* email,
                                               int type,
                                               const char* pin)
{
...
{
    // [<code>,"<email>","<ip_address>",<timestamp>,"<user_handle>",["<email>"]]   (and we are
...
CommandValidatePassword::CommandValidatePassword(MegaClient* client,
                                                 const char* email,
                                                 const vector<byte>& authKey)
{
    cmd("us");
    arg("user", email);
...
bool CommandValidatePassword::procresult(Result r, JSON&)
{
    ...
    {
        assert(r.hasJsonObject()); // we don't use the object contents, and will exit the object
                                   // automatically
...
CommandGetEmailLink::CommandGetEmailLink(MegaClient* client,
                                         const char* email,
                                         int add,
                                         const char* pin)
{
...
CommandConfirmEmailLink::CommandConfirmEmailLink(MegaClient* client,
                                                 const char* code,
                                                 const char* email,
                                                 const byte* newLoginHash,
                                                 bool replace)
{
    this->email = email;
...
#ifdef ENABLE_CHAT
CommandChatCreate::CommandChatCreate(MegaClient* client,
                                     bool group,
                                     bool publicchat,
                                     const userpriv_vector* upl,
                                     const string_map* ukm,
                                     const char* title,
                                     bool meetingRoom,
                                     int chatOptions,
                                     const ScheduledMeeting* schedMeeting)
{
    ...
    if (group)
    {
        ...
        if (mChatOptions.speakRequest())
        {
            arg("sr", 1);
        }
        if (mChatOptions.waitingRoom())
        {
            arg("w", 1);
        }
        if (mChatOptions.openInvite())
        {
            arg("oi", 1);
        }
    }
    ...
    {
        mSchedMeeting.reset(
            schedMeeting->copy()); // can avoid copy by mooving check from where we are calling
        beginobject("sm");
        arg("a", "mcsmp");
    ...
    else
    {
        ...
        while (!exit)
        {
            switch (json.getnameid())
            {
                ...
                case makeNameid("sm"):
                {
                    ...
                    {
                        LOG_err << "Error creating a scheduled meeting along with chat. chatId ["
        ...
        if (chatid != UNDEF && shard != -1)
        {
            if (addSchedMeeting)
            {
                ...
                {
                    LOG_err
                        << "Scheduled meeting id received upon mcc command, but there's no local "
                           "scheduled meeting data. chatId ["
                        << toHandle(chatid) << "]";
                    addSchedMeeting = false;
                    assert(false);
                }
            ...
            // no need to fetch scheduled meetings as we have just created the chat, so it doesn't
            // have any

            if (group) // we are creating a chat, so we need to initialize all chat options
                       // enabled/disabled
            {
                chat->addOrUpdateChatOptions(mChatOptions.speakRequest(),
                                             mChatOptions.waitingRoom(),
                                             mChatOptions.openInvite());
            }
            ...
            {
                LOG_err << "Error adding a new scheduled meeting with schedId ["
...
CommandSetChatOptions::CommandSetChatOptions(MegaClient* client,
                                             handle chatid,
                                             int option,
                                             bool enabled,
                                             CommandSetChatOptionsCompletion completion):
    mCompletion(completion)
{
    this->client = client;
    mChatid = chatid;
    ...
    switch (option)
    {
        case ChatOptions::kOpenInvite:
            arg("oi", enabled);
            break;
        case ChatOptions::kSpeakRequest:
            arg("sr", enabled);
            break;
        case ChatOptions::kWaitingRoom:
            arg("w", enabled);
            break;
        default:
            break;
...
CommandChatInvite::CommandChatInvite(MegaClient* client,
                                     handle chatid,
                                     handle uh,
                                     privilege_t priv,
...
CommandChatGrantAccess::CommandChatGrantAccess(MegaClient* client,
                                               handle chatid,
                                               handle h,
                                               const char* uid)
{
...
CommandChatRemoveAccess::CommandChatRemoveAccess(MegaClient* client,
                                                 handle chatid,
                                                 handle h,
                                                 const char* uid)
{
...
CommandChatUpdatePermissions::CommandChatUpdatePermissions(MegaClient* client,
                                                           handle chatid,
                                                           handle uh,
                                                           privilege_t priv)
{
...
CommandRegisterPushNotification::CommandRegisterPushNotification(MegaClient* client,
                                                                 int deviceType,
                                                                 const char* token)
{
    this->client = client;
...
CommandSetChatRetentionTime::CommandSetChatRetentionTime(MegaClient* client,
                                                         handle chatid,
                                                         unsigned period)
{
    mChatid = chatid;
...
bool CommandChatLinkURL::procresult(Result r, JSON& json)
{
    if (r.wasStrictlyError())
    {
        client->app->chatlinkurl_result(UNDEF,
                                        -1,
                                        NULL,
                                        NULL,
                                        -1,
...
CommandChatLinkJoin::CommandChatLinkJoin(MegaClient* client,
                                         handle publichandle,
                                         const char* unifiedkey)
{
    cmd("mciph");
...
CommandGetMegaAchievements::CommandGetMegaAchievements(MegaClient* client,
...
bool CommandContactLinkQuery::procresult(Result r, JSON& json)
{
    ...
    {
        client->app
            ->contactlinkquery_result(r.errorOrOK(), h, &email, &firstname, &lastname, &avatar);
        return true;
    }
    ...
    for (;;)
    {
        switch (json.getnameid())
        {
            ...
            case EOO:
                client->app
                    ->contactlinkquery_result(API_OK, h, &email, &firstname, &lastname, &avatar);
                return true;
            default:
                if (!json.storeobject())
                {
                    LOG_err << "Failed to parse query contact link response";
                    client->app->contactlinkquery_result(API_EINTERNAL,
                                                         h,
                                                         &email,
                                                         &firstname,
                                                         &lastname,
...
bool CommandMultiFactorAuthSetup::procresult(Result r, JSON& json)
{
    ...
    else if (r.wasErrorOrOK()) //[0] is valid response (returned when mfa is supplied in the
                               // request)
    {
    ...
    for (;;)
    {
        switch (json.getnameid())
        {
            ...
            case EOO:
                ...
                client->app->getpsa_result(API_OK,
                                           id,
                                           &title,
                                           &text,
                                           &imagepath,
            ...
            default:
                if (!json.storeobject())
                {
                    ...
                    client->app
                        ->getpsa_result(API_EINTERNAL, 0, NULL, NULL, NULL, NULL, NULL, NULL);
                    return false;
                }
                break;
        }
...
CommandFetchTimeZone::CommandFetchTimeZone(MegaClient* client,
                                           const char* timezone,
...
CommandSMSVerificationSend::CommandSMSVerificationSend(MegaClient* client,
                                                       const string& phoneNumber,
                                                       bool reVerifyingWhitelisted)
{
    cmd("smss");
    batchSeparately = true; // don't let any other commands that might get batched with it cause the
                            // whole batch to fail
...
CommandSMSVerificationCheck::CommandSMSVerificationCheck(MegaClient* client,
        ...
        while (!exit)
        {
            switch (json.getnameid())
            {
                ...
                case EOO:
                {
                    ...
                    {
                        LOG_err << "Missing or empty fields when parsing 'get country calling "
                                   "codes' response";
                        success = false;
                    }
                    else
                    {
                        countryCallingCodes.emplace(
                            make_pair(std::move(countryCode), std::move(callingCodes)));
                    }
                    exit = true;
                    break;
                }
        ...
        switch (json.getnameid())
        {
            ...
            case EOO:
                if (attr.empty())
                {
                    ...
                    client->app->folderlinkinfo_result(API_EINCOMPLETE,
                                                       UNDEF,
                                                       UNDEF,
                                                       NULL,
                                                       NULL,
                ...
                if (key.size() <= 9 || key.find(":") == string::npos)
                {
                    ...
                    client->app
                        ->folderlinkinfo_result(API_EKEY, UNDEF, UNDEF, NULL, NULL, 0, 0, 0, 0, 0);
                    return false;
                }
                if (parentHandle != ph)
                {
                    LOG_err << "Folder link information: public handle doesn't match";
                    client->app->folderlinkinfo_result(API_EINTERNAL,
                                                       UNDEF,
                                                       UNDEF,
                                                       NULL,
                                                       NULL,
...
CommandBackupPut::CommandBackupPut(MegaClient* client,
                                   const BackupInfo& fields,
                                   std::function<void(Error, handle /*backup id*/)> completion):
    mCompletion(completion)
{
    mSeqtagArray = true;
    ...
    if (!fields.localFolder.empty())
    {
        string localFolderEncrypted(
            client->cypherTLVTextWithMasterKey("lf", fields.localFolder.toPath(false)));
...
bool CommandBackupPut::procresult(Result r, JSON& json)
{
    if (r.hasJsonItem())
    {
        ...
        if (mCompletion)
            mCompletion(API_OK, backupId);
        client->app->backupput_result(API_OK, backupId);
        return true;
    }
    else if (r.wasErrorOrOK())
    {
        assert(r.errorOrOK() != API_EARGS); // if this happens, the API rejected the request because
                                            // it wants more fields supplied
        if (mCompletion)
...
CommandBackupPutHeartBeat::CommandBackupPutHeartBeat(MegaClient* client,
                                                     handle backupId,
                                                     SPHBStatus status,
                                                     int8_t progress,
                                                     uint32_t uploads,
...
CommandBackupSyncFetch::CommandBackupSyncFetch(
    std::function<void(const Error&, const vector<Data>&)> f):
    completion(std::move(f))
{
    cmd("sf");
}
...
bool CommandGetBanners::procresult(Result r, JSON& json)
{
    ...
    /*
       ...
       ///Banner description. "img": "Verified_image.png", ///Image name. "l": "", ///URL "bimg":
       "Verified_BG.png", ///background image name. "dsp": "https://domain/path" ///Where to get the
       image.
        }, {"id":3, ...}, ... ]
    */
    ...
    while (json.enterobject())
    {
        ...
        banners.emplace_back(make_tuple(id,
                                        std::move(title),
...
CommandRemoveSet::CommandRemoveSet(MegaClient* cl,
                                   handle id,
                                   std::function<void(Error)> completion):
    mSetId(id),
    mCompletion(completion)
...
CommandPutSetElements::CommandPutSetElements(
    MegaClient* cl,
    vector<SetElement>&& els,
    vector<StringPair>&& encrDetails,
    std::function<void(Error, const vector<const SetElement*>*, const vector<int64_t>*)>
        completion):
    mElements(new vector<SetElement>(std::move(els))),
    mCompletion(completion)
{
    mSeqtagArray = true;
    cmd("aepb");
...
bool CommandPutSetElements::procresult(Result r, JSON& json)
{
    ...
    for (size_t elCount = 0u; elCount < mElements->size(); ++elCount)
    {
        ...
        else if (json.enterobject())
        {
            ...
            {
                if (errEid == UNDEF)
                    LOG_warn << "Sets: Extended error missing Element id";
            }
            else
            {
                json.pos = posAux;
...
CommandPutSetElement::CommandPutSetElement(
    MegaClient* cl,
    SetElement&& el,
    unique_ptr<string> encrAttrs,
    string&& encrKey,
    std::function<void(Error, const SetElement*)> completion):
    mElement(new SetElement(std::move(el))),
    mCompletion(completion)
{
    mSeqtagArray = true;
    cmd("aep");
...
bool CommandPutSetElement::procresult(Result r, JSON& json)
{
    ...
    bool parsedOk = procerrorcode(r, e) || procresultid(json,
                                                        r,
                                                        elementId,
                                                        ts,
                                                        nullptr,
...
CommandRemoveSetElements::CommandRemoveSetElements(
    MegaClient* cl,
    handle sid,
    vector<handle>&& eids,
    std::function<void(Error, const vector<int64_t>*)> completion):
    mSetId(sid),
    mElemIds(std::move(eids)),
    mCompletion(completion)
{
    cmd("aerb");
...
bool CommandRemoveSetElements::procresult(Result r, JSON& json)
{
    ...
    for (size_t elCount = 0u; elCount < mElemIds.size(); ++elCount)
    {
        ...
        else if (json.enterobject())
        {
            ...
            {
                if (errEid == UNDEF)
                    LOG_warn << "Sets: Extended error missing Element id in `aerb`";
            }
            else
            {
                jsonOk = false;
...
CommandRemoveSetElement::CommandRemoveSetElement(MegaClient* cl,
                                                 handle sid,
                                                 handle eid,
                                                 std::function<void(Error)> completion):
    mSetId(sid),
...
CommandExportSet::CommandExportSet(MegaClient* cl,
                                   Set&& s,
                                   bool makePublic,
                                   std::function<void(Error)> completion):
    mSet(new Set(std::move(s))),
...
bool CommandExportSet::procresult(Result r, JSON& json)
{
    ...
    const bool parsedOk =
        procerrorcode(r, e) ||
        procresultid(json, r, sid, ts, nullptr, nullptr, nullptr, nullptr, &publicId);
...
CommandMeetingStart::CommandMeetingStart(MegaClient* client,
                                         const handle chatid,
                                         const bool notRinging,
                                         CommandMeetingStartCompletion completion):
    mCompletion(completion)
{
    cmd("mcms");
    arg("cid", (byte*)&chatid, MegaClient::CHATHANDLE);
...
CommandMeetingJoin::CommandMeetingJoin(MegaClient* client,
                                       handle chatid,
                                       handle callid,
                                       CommandMeetingJoinCompletion completion):
    mCompletion(completion)
{
    cmd("mcmj");
    arg("cid", (byte*)&chatid, MegaClient::CHATHANDLE);
...
CommandMeetingEnd::CommandMeetingEnd(MegaClient* client,
                                     handle chatid,
                                     handle callid,
                                     int reason,
                                     CommandMeetingEndCompletion completion):
    mCompletion(completion)
{
    cmd("mcme");
    arg("cid", (byte*)&chatid, MegaClient::CHATHANDLE);
...
CommandRingUser::CommandRingUser(MegaClient* client,
                                 handle chatid,
                                 handle userid,
                                 CommandRingUserCompletion completion):
    mCompletion(completion)
{
    cmd("mcru");
...
CommandScheduledMeetingAddOrUpdate::CommandScheduledMeetingAddOrUpdate(
    MegaClient* client,
    const ScheduledMeeting* schedMeeting,
    const char* chatTitle,
    CommandScheduledMeetingAddOrUpdateCompletion completion):
    mScheduledMeeting(schedMeeting->copy()),
    mCompletion(completion)
{
    assert(schedMeeting);
    cmd("mcsmp");
...
bool CommandScheduledMeetingAddOrUpdate::procresult(Command::Result r, JSON& json)
{
    ...
    {
        if (mScheduledMeeting)
        {
            client->reportInvalidSchedMeeting(mScheduledMeeting.get());
        }
        if (mCompletion)
        {
            mCompletion(API_EINTERNAL, nullptr);
    ...
    const bool added = chat->addOrUpdateSchedMeeting(std::unique_ptr<ScheduledMeeting>(
        mScheduledMeeting->copy())); // add or update scheduled meeting if already exists
    ...
    if (mCompletion)
    {
        mCompletion(added ? API_OK : API_EINTERNAL, mScheduledMeeting.get());
    }
    return true;
}

CommandScheduledMeetingRemove::CommandScheduledMeetingRemove(
    MegaClient* client,
    handle chatid,
    handle schedMeeting,
    CommandScheduledMeetingRemoveCompletion completion):
    mChatId(chatid),
    mSchedId(schedMeeting),
    mCompletion(completion)
{
    cmd("mcsmr");
...
{
    cmd("mcsmf");
    if (schedMeeting != UNDEF)
    {
        arg("id", (byte*)&schedMeeting, MegaClient::CHATHANDLE);
    }
    if (chatid != UNDEF)
    {
        arg("cid", (byte*)&chatid, MegaClient::CHATHANDLE);
...
bool CommandScheduledMeetingFetch::procresult(Command::Result r, JSON& json)
{
    ...
    if (mCompletion)
    {
        mCompletion(err, err == API_OK ? &schedMeetings : nullptr);
    }
    return err == API_OK;
}

CommandScheduledMeetingFetchEvents::CommandScheduledMeetingFetchEvents(
    MegaClient* client,
    handle chatid,
    m_time_t since,
    m_time_t until,
    unsigned int count,
    bool byDemand,
    CommandScheduledMeetingFetchEventsCompletion completion):
    mChatId(chatid),
    mByDemand(byDemand),
    mCompletion(completion ? completion :
...
bool CommandScheduledMeetingFetchEvents::procresult(Command::Result r, JSON& json)
{
    ...
    {
        chat->addUpdatedSchedMeetingOccurrence(
            std::unique_ptr<ScheduledMeeting>(schedMeeting->copy()));
    }

    // set the change type although we haven't received any occurrences (but there's no error and
    // json proccessing has been succesfull)
    if (mByDemand)
    {
...
CommandFetchAds::CommandFetchAds(MegaClient* client,
                                 int adFlags,
                                 const std::vector<std::string>& adUnits,
                                 handle publicHandle,
                                 CommandFetchAdsCompletion completion):
    mCompletion(completion),
    mAdUnits(adUnits)
{
    cmd("adf");
...
CommandQueryAds::CommandQueryAds(MegaClient* client,
                                 int adFlags,
                                 handle publicHandle,
                                 CommandQueryAdsCompletion completion):
    mCompletion(completion)
{
    cmd("ads");
    arg("ad", adFlags);
    ...
    MapClusterPublicKeys mapClusterPubKeys;
    {
        ...
        if (json.enterobject())
        {
            ...
            while (parsedOk)
            {
                ...
                {
                    LOG_err << "[CommandGetVpnCredentials] Could not convert param SlotID("
                            << slotIDStr << ") to integer. Exception: " << ex.what();
                    parsedOk = false;
                }
                ...
                if (parsedOk && json.enterarray())
                {
                    ...
                    if (parsedOk)
                    {
                        mapSlotIDToCredentialInfo.emplace(
                            std::make_pair(slotID, std::move(credentialInfo)));
                    }
                    json.leavearray();
                }
            }
        ...
        if (json.enterobject())
        {
            ...
            while (parsedOk)
            {
                ...
                {
                    LOG_err << "[CommandGetVpnCredentials] Could not convert param ClusterID("
                            << clusterIDStr << ") to integer. Exception: " << ex.what();
                    parsedOk = false;
                }
            ...
            if (!parsedOk)
            {
                // There were credentials and a valid ClusterID, but something was wrong with the
                // Cluster Public Key value
                if (mCompletion)
                {
                    mCompletion(e, {}, {}, {});
                }
        ...
        {
            // There were credentials, but there were no information regarding the Cluster Public
            // Key(s)
            if (mCompletion)
            {
                mCompletion(e, {}, {}, {});
...
bool CommandPutVpnCredential::procresult(Command::Result r, JSON& json)
{
    ...
    // We receive directly one array here (like in CommandGetVpnRegions), so we are inside the array
    // already
...
CommandCheckVpnCredential::CommandCheckVpnCredential(MegaClient* client,
                                                     string&& userPubKey,
                                                     Cb&& completion)
{
    cmd("vpnc");
...
CommandFetchCreditCard::CommandFetchCreditCard(MegaClient* client,
                                               CommandFetchCreditCardCompletion completion):
    mCompletion(std::move(completion))
{
    assert(mCompletion);
    cmd("cci");
...
CommandCreatePasswordManagerBase::CommandCreatePasswordManagerBase(
    MegaClient* cl,
    std::unique_ptr<NewNode> nn,
    int ctag,
    CommandCreatePasswordManagerBase::Completion&& cb):
    mNewNode(std::move(nn)),
    mCompletion(std::move(cb))
{
    mSeqtagArray = true;
    ...
    arg("k",
        reinterpret_cast<const byte*>(mNewNode->nodekey.data()),
        ...
        switch (json.getnameid())
        {
            ...
            case EOO:
            {
                ...
                const std::string msg{
                    "Password Manager: wrong node type received in command response. Received "};
                ...
                {
                    LOG_warn << msg << "unexpected key field value |" << key
                             << "| missing separator ':'."
                ...
                if (mCompletion)
                    mCompletion(API_OK, std::move(mNewNode));
                return true;
            }
            default:
                if (!json.storeobject())
